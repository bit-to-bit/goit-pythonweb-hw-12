...
def test_read_reset_password_email(client, monkeypatch):
    mock_send_email = Mock()
    monkeypatch.setattr("src.services.email.send_email", mock_send_email)
    response = client.post(
        "api/auth/reset_password_email_form",
    ...
    assert (
        data["message"]
        == "Перевірте свою електронну пошту для підтвердження скидання пароля"
    )
